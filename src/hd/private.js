...
function tapTweakHash(pubKey, h) {
    return luckycoinjs_lib_1.crypto.taggedHash("TapTweak", Buffer.concat(h ? [pubKey, h] : [pubKey]));
}
function tweakSigner(signer, opts) {
    ...
    const tweakedPrivateKey = tinysecp.privateAdd(privateKey, tapTweakHash((0, util_1.toXOnly)(signer.publicKey), opts.tweakHash));
...
class HDPrivateKey extends base_1.BaseWallet {
    ...
    findAccount(account) {
        ...
        const foundAccount = this.accounts.find((f) => this.getAddress(f.publicKey) === account);
    ...
    signPsbt(psbt, inputs) {
        ...
        inputs.forEach((input) => {
            ...
            if ((this.addressType === types_1.AddressType.P2TR ||
    ...
    signAllInputsInPsbt(psbt, accountAddress, disableTweakSigner) {
        ...
        psbt.data.inputs.forEach((input, idx) => {
            if ((this.addressType === types_1.AddressType.P2TR ||
                ...
                psbt.signInput(idx, signer, input.sighashType !== undefined ? [input.sighashType] : undefined);
            }
            else {
                const signer = account;
                psbt.signInput(idx, signer, input.sighashType !== undefined ? [input.sighashType] : undefined);
    ...
    fromSeed(opts) {
        this.childIndex = 0;
        this.seed = opts.seed;
        this.hdWallet = browser_hdkey_1.default.fromMasterSeed(Buffer.from(opts.seed));
        if (opts.hdPath) {
    ...
    async fromMnemonic(opts) {
        const seed = await (0, nintondo_bip39_1.mnemonicToSeed)(opts.mnemonic, opts.passphrase ?? "lky");
        this.fromSeed({
            seed,
    ...
    static deserialize(opts) {
        if (opts.numberOfAccounts === undefined || !opts.seed) {
            throw new Error("HDPrivateKey: Deserialize method cannot be called with an opts value for numberOfAccounts and no seed");
        }
