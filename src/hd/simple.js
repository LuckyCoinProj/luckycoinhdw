...
class HDSimpleKey extends base_1.BaseWallet {
    ...
    verifyMessage(_address, text, sig) {
        this.initPair();
        return this.pair.verify(Buffer.from((0, utils_1.hexToBytes)(text)), Buffer.from((0, utils_1.hexToBytes)(sig)));
    }
    getAccounts() {
        this.initPair();
    ...
    serialize() {
        ...
        if (!wif)
            throw new Error("Failed to export wif for simple wallet");
        return {
            privateKey: wif,
    ...
    signAllInputsInPsbt(psbt, accountAddress, disableTweakSigner) {
        ...
        if (accountAddress !== this.getAddress(this.publicKey))
            throw new Error("Provided account address does not match the wallet's address");
        psbt.data.inputs.forEach((input, idx) => {
            if ((this.addressType === types_1.AddressType.P2TR ||
                ...
                psbt.signInput(idx, this.pair, input.sighashType !== undefined ? [input.sighashType] : undefined);
            }
        });
        return {
